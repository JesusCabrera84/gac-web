<script>
    ...
    async function handleSubmit(e) {
        ...
        try {
            ...
            errorMessage =
                error.message || "Error al actualizar el dispositivo";
        } finally {
            isSaving = false;
        }
    }
    ...
    <div class="p-8 max-w-2xl mx-auto w-full">
        <Card class="p-8">
            ...
            {:else}
                {#if errorMessage}
                    <div
                        class="bg-red-50 text-red-600 p-4 rounded-md mb-6 text-sm"
                    >
                        {errorMessage}
                    </div>
                {/if}
                ...
                {#if successMessage}
                    <div
                        class="bg-green-50 text-green-600 p-4 rounded-md mb-6 text-sm"
                    >
                        {successMessage}
                    </div>
                {/if}
                ...
                <form onsubmit={handleSubmit} class="space-y-6">
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div class="col-span-2">
                            ...
                            >
                            <div
                                class="p-2 bg-slate-100 rounded text-slate-900 font-mono"
                            >
                                {deviceId}
                            </div>
                        </div>
                    ...
                    <div class="flex justify-end space-x-4 pt-4">
                        <Button
                            variant="ghost"
                            type="button"
                            onclick={() => goto("/products/nexus/devices")}
                        >
                            Volver
                        </Button>
                        <Button
