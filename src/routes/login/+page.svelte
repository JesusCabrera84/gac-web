...
<div class="w-full max-w-md relative z-10">
	<div class="text-center mb-8 flex flex-col items-center">
		...
		<p class="text-emerald-400/80 mt-2 text-sm uppercase tracking-widest">Admin Console</p>
	</div>
	...
	>
		<form onsubmit={handleLogin} class="space-y-6">
			<div class="space-y-1">
				<label for="email" class="block text-sm font-medium text-emerald-100/80"
			...
			<div class="space-y-1">
				<label for="password" class="block text-sm font-medium text-emerald-100/80"
					>Contraseña</label
				>
				<input
					id="password"
			...
			{#if error}
				<div class="p-3 rounded-md bg-red-950/30 border border-red-900/50 text-red-200 text-sm">
					{error}
				</div>
			{/if}
